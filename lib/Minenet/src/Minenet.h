...
class MinenetProtocol {
   public:
    ...
    enum TxState { TX_BURST = 0, TX_BURST_SPACE = 1, TX_DATA = 2, TX_BIT_MARK = 3, TX_COMPLETE = 4 };

    enum RxState { RX_BURST = 0, RX_BURST_SPACE = 1, RX_DATA = 2 };

   private:
    volatile bool txActive;
    volatile TxState txState;
    volatile uint8_t txSeq;
    volatile int8_t txBits;
    volatile uint32_t txShift;

    volatile RxState rxState;
    volatile int8_t rxBits;
    volatile uint32_t rxShift;
    volatile bool rxAvailable;
};
